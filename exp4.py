...
def search_with_bm25(query):
    ...
    doc_freq = FreqDist(
        [token for doc in results for token in preprocess_text(doc['Title'])])
    N = len(results)
...
def search_with_language_model(query):
    ...
    for doc in results:
        doc_text = preprocess_text(doc['Title'])
        doc_len = len(doc_text)
        lm_score = calculate_language_model(
            query_tokens, doc_text, doc_len, collection_freq, mu=0.7)
...
with open('nfcorpus/train.titles.queries', 'r', encoding='utf-8') as file:
    for l in file:
        ...
        for rank, (doc_id, score) in enumerate(lm_results, start=1):
            print(
                f"Rank {rank}: Document ID: {doc_id}, Language Model Score: {score}")
